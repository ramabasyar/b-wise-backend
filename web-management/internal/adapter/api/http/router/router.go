...
func SetupWithLogger(
	...
	if h.Public != nil {
		...
		pub.GET("/preview/:token", h.Public.Preview)
	...
	if h.Content != nil {
		...
		posts.Use(jwksAuth.RequireAuth(), permCheck.CheckAccess())
		{
			...
			posts.POST("/:id/publish", permCheck.RequirePermission("contents.publish"), h.Content.PublishPost)
			...
			posts.POST("/:id/submit-review", permCheck.RequirePermission("contents.write"), h.Content.SubmitReview)
			posts.POST("/:id/approve", permCheck.RequirePermission("contents.publish"), h.Content.ApprovePost)
			posts.POST("/:id/reject", permCheck.RequirePermission("contents.publish"), h.Content.RejectPost)
			posts.GET("/:id/reviews", permCheck.RequirePermission("contents.read"), h.Content.ListReviews)
			posts.POST("/:id/preview-token", permCheck.RequirePermission("contents.write"), h.Content.PreviewToken)
